...
// origin: "http://localhost:5501", // Erlaubt nur Anfragen von dieser URL
...
app.get("/list/search", (req, res) => {
  ...
  const filteredList = listlog.filter((tdList) =>
    tdList.listName.toLowerCase().includes(listName.toLowerCase())
  );
...
app.get("/task/search", (req, res) => {
  ...
  const filteredTask = listlog.find((tdList) =>
    tdList.tasks.find((task) =>
...
app.post("/createTask/:listName", (req, res) => {
  ...
  const foundedList = listlog.find(
    (tdList) => tdList.listName.toLowerCase() === listName.toLowerCase()
  );
  const foundedTask = foundedList.tasks.find(
...
app.put("/updateCheckbox/:listName/:taskName", (req, res) => {
  ...
  const foundedList = listlog.find((tdList) =>
    tdList.tasks.find(
      (task) => task.taskName.toLowerCase() === taskName.toLowerCase()
    )
  );
...
app.delete("/deleteList/:listName", (req, res) => {
  const listlog = readFile();
  const listName = req.params.listName;
  const filteredList = listlog.filter(
    (tdList) => tdList.listName.toLowerCase() !== listName.toLowerCase()
  );
...
app.delete("/deleteTask/:listName/:taskName", (req, res) => {
  ...
  foundedList.tasks = foundedList.tasks.filter(
    (task) => task.taskName.toLowerCase() !== taskName.toLowerCase()
  );
  foundedList.dateOfUpdate = formattedDate;
  writeFile(listlog);
